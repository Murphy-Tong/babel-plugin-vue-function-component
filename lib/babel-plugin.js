...
function resolveJSType(node) {
    ...
    if (t.isTSArrayType(node) || t.isTSTupleType(node) ||
...
function createSetupExpression(name, fn, state) {
    const propsTypeMap = fn.params?.[0]
        ? resolveTSMembers(fn.params[0].typeAnnotation?.typeAnnotation, state)
        : null;
    const propsObj = [];
    ...
    const defineCallee = state.vueNSName
        ? `${state.vueNSName}.${state.defineComponentLocalName || "defineComponent"}`
...
const fnVisitor = {
    ...
    ExportNamedDeclaration: function (path, state) {
        ...
        if (t.isFunctionDeclaration(declaration)) {
            ...
            path.get("declaration").replaceWith(createComponentVariableDeclaration(declaration.id.name, createSetupExpression(declaration.id.name, declaration, state)));
        }
...
function default_1() {
    return {
        inherits: function (api, opt, dir) {
            return (0, plugin_syntax_typescript_1.default)(api, { ...opt || {}, isTSX: true }, dir);
        },
        post: function () {
            const dir = path_1.default.resolve(__dirname, "../transformed");
            fs.mkdirSync(dir.toString(), { recursive: true });
            fs.promises.writeFile(path_1.default.resolve(dir, "out.json"), JSON.stringify(transformedFns, null, 4));
        },
        visitor: {
            Program: {
                enter(path, state) {
                    ...
                    for (let i = 0; i < imports.length; i++) {
                        ...
                        for (let j = 0; j < specifiers.length; j++) {
                            ...
                            if (t.isImportSpecifier(specifier)) {
                                ...
                                else if (specifier.imported.name ===
                                    "defineComponent") {
