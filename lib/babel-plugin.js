...
`);
const SETUP_TEMPLATE_NS = template_1.default.expression(`
NS.CALLEE({
    name:NAME,
    props:PROPS_NODE,
    setup:SETUP_FN
...
function processVariableDeclaration(path, state) {
    ...
    path.node.declarations.forEach((declare, index) => {
        ...
        if (!(0, config_1.canProcessFn)(val, state.filename, name, state.includeFns)) {
            return;
        }
...
function default_1() {
    return {
        ...
        visitor: {
            Program: {
                enter(path, state) {
                    if (!(0, config_1.canProcessFile)(state.filename, state.includeFiles)) {
                        path.stop();
                        return;
                    }
                    setErrorBuilder(state, path);
                    Object.assign(state, { ...config_1.DEFAULT_CONFIG, ...state });
            ...
            FunctionDeclaration: function (path, state) {
                setErrorBuilder(state, path);
                if (!(0, config_1.canProcessFn)(path.node, state.filename, path.node.id.name, state.includeFns)) {
                    path.skip();
                    return;
                }
                // 顶级的方法声明肯定有名字
            ...
            ExportDefaultDeclaration: function (path, state) {
                ...
                if (t.isFunctionDeclaration(declaration)) {
                    if (!(0, config_1.canProcessFn)(declaration, state.filename, declaration.id?.name || 'default', state.includeFns)) {
                        path.skip();
                        return;
                    }
            ...
            ExportNamedDeclaration: function (path, state) {
                ...
                const declaration = path.node.declaration;
                if (t.isFunctionDeclaration(declaration)) {
                    if (!(0, config_1.canProcessFn)(declaration, state.filename, declaration.id.name, state.includeFns)) {
                        path.skip();
                        return;
                    }
                    //方法声明肯定有名字的
