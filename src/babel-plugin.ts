...
function resolveJSType(node: t.TSType): string | null {
  ...
  if (
    t.isTSArrayType(node) || t.isTSTupleType(node) ||
    (t.isTSTypeReference(node) &&
      ((node as t.TSTypeReference).typeName as t.Identifier).name === "Array")
  ) {
    return "Array";
  }
  ...
  if (t.isTSLiteralType(node)) {
    // field: "A"|1|false
    if (
      t.isTemplateLiteral(node.literal) || t.isUnaryExpression(node.literal)
    ) {
      return null;
    }
  ...
  if (
    t.isTSAnyKeyword(node) || t.isTSTypeReference(node) ||
...
function resolveTSMembersFromTSObject(node: t.TSTypeLiteral, state: IState) {
  ...
  for (let member of node.members) {
    if (t.isTSPropertySignature(member)) {
      ...
      propsMap[name] =
        (member.typeAnnotation as t.TSTypeAnnotation).typeAnnotation;
    } else {
...
function resolveTSMembersFromTSInterface(
  ...
  node.body.forEach((ele) => {
    ...
    propsMap[(ele.key as t.Identifier).name] =
      ele.typeAnnotation!.typeAnnotation;
  });
  return propsMap;
}
...
function resolveTSMembersFromTSReference(
  ...
  if (!typeDeclared) {
    throw state.buildCodeFrameError!(
      "当前文件中找不到定义的类型:" + (node.typeName as t.Identifier).name,
    );
  }
...
function createSetupExpression(
  name: string,
  fn: t.FunctionDeclaration | t.FunctionExpression | t.ArrowFunctionExpression,
  state: IState,
): t.Expression {
  const propsTypeMap: Record<string, t.TSType> | null = fn.params?.[0]
    ? resolveTSMembers(
      (fn.params[0].typeAnnotation as t.TSTypeAnnotation)?.typeAnnotation,
      state,
    )
    : null;
  const propsObj: t.ObjectProperty[] = [];
  const propType = state.vueNSName
    ? `${state.vueNSName}.PropType`
  ...
  if (propsTypeMap) {
    Object.entries(propsTypeMap).forEach(([name, node]) => {
      ...
      if (tsType && isPrimaryType(tsType)) {
        propsObj.push(
          t.objectProperty(t.identifier(name), t.identifier(tsType)),
        );
      } else {
        propsObj.push(
          t.objectProperty(
            ...
            t.tsAsExpression(
              ...
              t.tSTypeReference(
                ...
                t.tsTypeParameterInstantiation([node]),
              ),
            ),
          ),
        );
  ...
  const defineCallee = state.vueNSName
    ? `${state.vueNSName}.${
  ...
  return (!propsObj.length ? SETUP_TEMPLATE_NO_PROPS : SETUP_TEMPLATE)(
    placements,
  );
}
  ...
  path.node.declarations.forEach((declare, index) => {
    ...
    path.insertAfter(
      createComponentVariableDeclaration(
        name,
        createSetupExpression(name, val, state),
      ),
    );
  });
  if (!declarationsCopy.length) {
...
function addVueImport(path: NodePath<t.Program>, state: IState) {
  ...
  if (!insertTarget) {
    insertTarget = t.importDeclaration([
      ...
      t.importSpecifier(t.identifier("PropType"), t.identifier("PropType")),
    ], t.stringLiteral(state.opts!.vue!));
...
let transformedFns: Record<string, string[]> = {};
function onFnTransformed(
  state: IState,
  ...
  let f = transformedFns[state.filename];
  if (!f) {
    f = [];
    transformedFns[state.filename] = f;
  }
  f.push(fnName || "default");
}
...
const fnVisitor = {
  FunctionDeclaration: function (
    ...
    path.replaceWith(
      createComponentVariableDeclaration(
        path.node.id!.name,
        createSetupExpression(path.node.id!.name, path.node, state),
      ),
    );
    path.skip();
  },
  ...
  ExportDefaultDeclaration: function (
    ...
    if (t.isFunctionDeclaration(declaration)) {
      ...
      onFnTransformed(
        state,
        path,
        declaration.id?.name || fspath.parse(state.filename).name,
      );
      path.get("declaration").replaceWith(
        createSetupExpression(
          declaration.id?.name || fspath.parse(state.filename).name,
          declaration,
          state,
        ),
  ...
  ExportNamedDeclaration: function (
    ...
    if (t.isFunctionDeclaration(declaration)) {
      ...
      processVariableDeclaration(
        path.get("declaration") as NodePath<t.VariableDeclaration>,
        state,
      );
    }
    path.skip();
  ...
  return {
    ...
    post: function () {
      ...
      fs.promises.writeFile(
        fspath.resolve(dir, "out.json"),
        JSON.stringify(transformedFns, null, 4),
      );
    },
    visitor: {
      Program: {
        enter(path: NodePath<t.Program>, state: IState) {
          ...
          const others = path.node.body.filter((segment) =>
            !t.isImportDeclaration(segment)
          );
          if (!others.length) {
            // 没有需要处理的东西
            path.stop();
          ...
          const imports = path.node.body.filter((segment) =>
            t.isImportDeclaration(segment)
          );
